...
function mergeRow(u, ent, act, fnd){
  ...
  const status=fnd?('Founding #'+fnd.number):(plan!=='free'?'Ödənişli':'Pulsuz');
  var ls='FREE';if(plan==='blocked')ls='BLOCKED';else if(plan==='active'||plan==='premium'||plan==='go'||fnd)ls='ACTIVE';else if((act&&act.testsCompleted>=5)||(act&&act.aiMessages>=15))ls='LIMIT_REACHED';
  return { uid:u.uid, email:u.email||'',
    ...
    lastSeen:(act&&act.lastSeen)||null, plan, status, limitStatus:ls,
    testsCompleted:(act&&act.testsCompleted)||0, aiMessages:(act&&act.aiMessages)||0,
...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    if(mode==='activate'&&q.uid){await db.doc('entitlements/'+String(q.uid)).set({plan:'active',activatedAt:Date.now()},{merge:true});return res.json({ok:true});}
    if(mode==='block'&&q.uid){await db.doc('entitlements/'+String(q.uid)).set({plan:'blocked',blockedAt:Date.now()},{merge:true});return res.json({ok:true});}
    if(mode==='deactivate'&&q.uid){await db.doc('entitlements/'+String(q.uid)).set({plan:'free',deactivatedAt:Date.now()},{merge:true});return res.json({ok:true});}
